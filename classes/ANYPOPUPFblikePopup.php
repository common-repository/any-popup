...
class ANYPOPUPFblikePopup extends ANYPOPUP
{
	public $content;
	...
	public function save($data = array())
	{
		...
		$anypopupFblikeContent = $this->getContent();
		$fblikeOptions = $this->getFblikeOptions();

		global $wpdb;
		if ($editMode) {
			$anypopupFblikeContent = stripslashes($anypopupFblikeContent);
			$sql = $wpdb->prepare("UPDATE ".$wpdb->prefix."anypopup_fblike_popup SET content=%s, options=%s WHERE id=%d", $anypopupFblikeContent, $fblikeOptions, $this->getId());
			$res = $wpdb->query($sql);
		}
		else {

			$sql = $wpdb->prepare("INSERT INTO ".$wpdb->prefix."anypopup_fblike_popup (id, content, options) VALUES (%d, %s, %s)",$this->getId(),$anypopupFblikeContent, $fblikeOptions);
			$res = $wpdb->query($sql);
		}
		return $res;
	}
	...
	{
		global $wpdb;
		$st = $wpdb->prepare("SELECT content, options FROM ".$wpdb->prefix."anypopup_fblike_popup WHERE id = %d", $id);
	...
	protected function getExtraRenderOptions()
	{
		...
		$content .= "<div id=\"anypopup-facebook-like\">
		...
		$content .= '<div class = "anypopup-fb-buttons-wrapper"><div class="fb-like" data-href="'.$url.'" data-layout="'.$layout.'" data-action="like" data-show-faces="true" data-share="'.$shareButtonStatus.'"></div></div></div>';
		$content .= '<style>
			.anypopup-fb-buttons-wrapper{
				text-align: center;
				min-height: 25px;
			}
			#anypopupcboxLoadedContent iframe {
