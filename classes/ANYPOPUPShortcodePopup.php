...
class ANYPOPUPShortcodePopup extends ANYPOPUP {
	private $shortcode;
	...
	public function save($data = array()) {
		...
		if ($editMode) {
			...
			$sqlUp = $wpdb->prepare("UPDATE ". $wpdb->prefix ."anypopup_shortCode_popup SET url=%s WHERE id=%d",$this->getShortcode(),$this->getId());
			$res = $wpdb->query($sqlUp);
		}
		else {
			$sql = $wpdb->prepare( "INSERT INTO ". $wpdb->prefix ."anypopup_shortCode_popup (id, url) VALUES (%d,%s)",$this->getId(),$this->getShortcode());
			$res = $wpdb->query($sql);
		}
		return $res;
	}
	protected function setCustomOptions($id) {
		...
		$st = $wpdb->prepare("SELECT * FROM ". $wpdb->prefix ."anypopup_shortCode_popup WHERE id = %d",$id);
