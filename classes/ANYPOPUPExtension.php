<?php
abstract class ANYPOPUPExtension {

	const ANYPOPUP_ADDON_TABLE_NAME = 'anypopup_addons';
	const ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME = 'anypopup_addons_connection';
	...
	public static function createExtensionTables($blogsId) {
		...
		$anypopupAddon = "CREATE TABLE IF NOT EXISTS ". $wpdb->prefix.$blogsId.ANYPOPUPExtension::ANYPOPUP_ADDON_TABLE_NAME." (
		...
		$extensionDataTable = "CREATE TABLE IF NOT EXISTS ". $wpdb->prefix.$blogsId.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME." (
			`id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$columInfo = $wpdb->query("SHOW COLUMNS FROM ".$wpdb->prefix.$blogsId.ANYPOPUPExtension::ANYPOPUP_ADDON_TABLE_NAME." LIKE 'isEvent'");

		if(!$columInfo) {
			$alterQuery = "ALTER TABLE ".$wpdb->prefix.$blogsId.ANYPOPUPExtension::ANYPOPUP_ADDON_TABLE_NAME." ADD isEvent TINYINT UNSIGNED NOT NULL";
	...
	public static function getSavedOptions($popupId, $extensionKey) {
		...
		$prepareSql = $wpdb->prepare("SELECT options FROM ".$wpdb->prefix.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME." WHERE popupId = %d AND extensionKey = %s", $popupId, $extensionKey);
	...
	private function isExistOptionData($popupId, $extensionKey) {
		...
		$prepareSql = $wpdb->prepare("SELECT COUNT(*) as count FROM ".$wpdb->prefix.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME." WHERE popupId = %d AND extensionKey=%s", $popupId, $extensionKey);
	...
	public function save() {
		...
		$extensionConnectionTable = ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME;
		$extensionKey = static::ANYPOPUP_EXTENSION_KEY;
		$extensionType = static::ANYPOPUP_EXTENSION_TYPE;
	...
	public static function getExtensionsOptions($popupId, $type = 'option') {
		...
		$prepareSql = $wpdb->prepare("SELECT extensionKey, options FROM ".$wpdb->prefix.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME." WHERE popupId = %d AND extensionType=%s", $popupId, $type);
	...
	public static function getPopupSavedExtensionsKeys($popupId) {
		...
		$prepareSql = $wpdb->prepare("SELECT extensionKey, paths FROM ".$wpdb->prefix."anypopup_addons LEFT JOIN ".$wpdb->prefix.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME ." ON extensionKey = name  and popupId = %d", $popupId);
		$extensionOptions = $wpdb->get_results($prepareSql, ARRAY_A);
	...
	public static function hasPopupEvent($popupId) {
		...
		$prepareSql = $wpdb->prepare("SELECT count(id) as count FROM ".$wpdb->prefix.static::ANYPOPUP_ADDON_TABLE_NAME ." WHERE isEvent = 1 AND name in (SELECT extensionKey FROM ".$wpdb->prefix.ANYPOPUPExtension::ANYPOPUP_ADDON_POPUP_CONNECTION_TABLE_NAME." where popupId = %d)", $popupId);
