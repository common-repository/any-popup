...
beckend.prototype.anypopupInit =  function() {
...
beckend.prototype.switchPopupActive = function() {
	...
	jQuery(".anypopup-switch-checkbox").bind('change', function() {
		var dataOptions = {};
...
beckend.prototype.colorPicekr = function() {
	var that = this;
	var anypopupColorPicker = jQuery('.anypopupOverlayColor').wpColorPicker({
		change: function() {
			var anypopupColorpicker = jQuery(this);
			that.changeColor(anypopupColorpicker);
		}
	});
	jQuery(".wp-picker-holder").bind('click',function() {
		var selectedInput = jQuery(this).prev().find('.anypopupOverlayColor');
...
beckend.prototype.changeColor = function(elemet) {
	...
	if(selectedName == 'countdownNumbersTextColor') {
		jQuery("#anypopup-counts-text").remove();
		jQuery("body").append("<style id=\"anypopup-counts-text\">.flip-clock-wrapper ul li a div div.inn { color: "+elementVal+"; }</style>");
	}
	if(selectedName == 'countdownNumbersBgColor') {
		jQuery("#anypopup-counts-style").remove();
		jQuery("body").append("<style id=\"anypopup-counts-style\">.flip-clock-wrapper ul li a div div.inn { background-color: "+elementVal+"; }</style>");
	}
};

beckend.prototype.reviewPopup = function () {

	jQuery('#anypopupcolorbox').ready(function () {
		jQuery('#anypopupcolorbox').on('anypopupCleanup', function () {
			var ajaxNonce = jQuery(this).attr('data-ajaxnonce');

			var data = {
				action: 'change_review_popup_show_period',
				ajaxNonce: ANYPOPUP_AJAX_NONCE
			};
			jQuery.post(ajaxurl, data, function(response,d) {
...
beckend.prototype.popupReview = function() {
	jQuery(".anypopup-dont-show-agin").on("click", function() {
		...
		jQuery( ".anypopup-info-panel-wrapper" ).hide(300);
	});

	jQuery('.anypopup-info-close').on('click', function() {
		jQuery( ".anypopup-info-panel-wrapper" ).hide(300);
	});
};
...
beckend.prototype.soundPreview = function () {
	...
	jQuery('.anypopup-preview-sound').bind('click', function () {
...
beckend.prototype.getPagesList = function() {
	
	jQuery(".anypopup-get-pages-checkbox").bind('change', function() {
		
		if(jQuery(this).is(":checked")) {
			$('#allSelectedPages').show();
			
			var popup_id = jQuery(this).attr("data-anypopup-popup-id");
			var ajaxNonce = jQuery(this).attr('data-checkbox-ajaxnonce');
			...
			jQuery.post(ajaxurl, data, function(response,d) {
				var pagesArray = jQuery.parseJSON(response);
		...
		return false;
			...
			$('#allSelectedPages').hide();
			$('#allSelectedPages').empty();
		}
	});
...
beckend.prototype.deletePopup = function() {
	jQuery(".anypopup-js-delete-link").bind('click',function() {
...
beckend.prototype.titleNotEmpty = function() {
	jQuery("#add-form").submit(function() {
		var popupTitle = jQuery(".anypopup-js-popup-title").val();
...
beckend.prototype.showThemePicture = function() {
	jQuery(".popup_theme_name").bind("mouseover",function(e) {
		jQuery('.theme'+jQuery(this).attr("anypopuppoupnumber")+'').css('display', 'block');
	});
};
...
beckend.prototype.showEffects = function() {
	...
	jQuery('select[name="effect"]').bind('change', function() {
		...
		jQuery("#effectShow").addClass('anypopup-animated '+jQuery(this).val()+'');
	});
	jQuery('.js-preview-effect').click(function() {
		...
		jQuery("#effectShow").removeClass();
		jQuery("#effectShow").show();
		jQuery("#effectShow").addClass('anypopup-animated '+jQuery('select[name="effect"] option:selected').val()+'');
	});
};
...
beckend.prototype.pageAcordion = function() {
	...
	acardionDivs(".generalTitle",'.anypopup_general_postbox','.generalContent');
	acardionDivs(".effectTitle",'.anypopup_effect_postbox','.effectsContent');
	acardionDivs(".optionsTitle",'.anypopup_options_postbox','.optionsContent');
	acardionDivs(".dimentionsTitle",'.anypopup_dimention_postbox','.dimensionsContent');
	acardionDivs(".js-advanced-title",'.js-advanced-postbox','.advanced-options-content');
	acardionDivs(".js-special-title",'.any-popup-special-postbox','.special-options-content');
};

beckend.prototype.fixedPostionSelection = function() {
	jQuery(".js-fixed-position-style").bind("click",function() {
		var anypopupelement = jQuery(this);
		var anypopuppos = anypopupelement.attr('data-anypopupvalue');
		jQuery(".js-fixed-position-style").css("backgroundColor","#FFFFFF");
		jQuery(this).css("backgroundColor","rgba(70,173,208,0.5)");
		jQuery(".js-fixed-postion").val(anypopuppos);
	});
	...
	jQuery(".js-fixed-position-style").bind("mouseover",function() {
		...
		jQuery(".js-fixed-position-style").each(function() {
			if (jQuery(this).attr("data-anypopupvalue") == jQuery('.js-fixed-postion').val())
				jQuery(this).css("backgroundColor","rgba(70,173,208,0.5)");
		});
	});

	jQuery(".js-fixed-position-style").bind("mouseout",function() {
		if(jQuery(".js-fixed-position-style").attr("data-anypopupvalue") !== jQuery(".js-fixed-postion").val() || jQuery(".js-fixed-postion").val() == 1) {
...
beckend.prototype.buildAccordionActions = function (currentRadioButton, event) {
	...
	if(event == 'change') {
		currentRadioButton.parents('.anypopup-radio-option-behavior').first().find('.js-radio-accordion').css({'display': 'none'});
	}
...
beckend.prototype.addCountries = function() {
	...
	if(typeof popupCountries != "undefined" && typeof popupCountries != "undefined"){
		var anypopupCountryNameArray = popupCountries.anypopupCountryName.split(",");
		var anypopupCountryIsoArray = popupCountries.anypopupCountryIso.split(",");
		for(var i=0; i <= anypopupCountryIsoArray.length; i++) {
			addCountry(anypopupCountryNameArray[i],anypopupCountryIsoArray[i]);
		}
	}
};
...
beckend.prototype.showCloseTextFieldForTheme = function() {
	...
	jQuery("[name='theme']").each(function() {
		if(jQuery(this).prop("checked")) {
			that.anypopupAllowCustomizedThemes(jQuery(this));
		}
	});
...
beckend.prototype.anypopupAllowCustomizedThemes = function(cureentRadioButton) {
	var customizedThemes = ['2','3','4'];
	var themeNumber = cureentRadioButton.attr("anypopuppoupnumber");
	var isInCustomThemes = customizedThemes.indexOf(themeNumber);
	jQuery(".themes-suboptions").addClass("anypopup-hide");
	if(isInCustomThemes != -1) {

		if(cureentRadioButton.prop( "checked" )) {
			jQuery(".anypopup-popup-theme-"+themeNumber).removeClass("anypopup-hide");
		}
		else {
			jQuery(".anypopup-popup-theme-"+themeNumber).addClass("anypopup-hide");
		}
	}
};
...
beckend.prototype.popupPreview = function () {
	var that = this;
	jQuery('.anypopup-popup-preview').bind('click', function (e) {
		e.preventDefault();
		...
		var newWindow = window.open(jQuery(this).attr('data-page-url'));
		jQuery.post(ajaxurl, data, function(response,d) {
			...
			var redirectUrl = that.updateQueryStringParameter(pageUrl, 'anypopup_id', popupId);
...
jQuery(document).ready(function($){
	var anypopupBeckeendObj = new  beckend();
	anypopupBeckeendObj.anypopupInit();
});
